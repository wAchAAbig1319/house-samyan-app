...
export const moviesApi = {
  ...
  getHomeFeed: () => {
    // แถวโปรโมชั่น — คำนวณอัตโนมัติจากหนังที่มี discountPercent > 0 ไม่ต้องไปเพิ่ม
    // id เองทุกครั้งที่ตั้งโปรโมชั่นใหม่ใน mockData.ts (ต่างจากแถวอื่นที่ fix ids ไว้)
    const promoMovies = MOCK_MOVIES.filter((m) => (m.discountPercent ?? 0) > 0);

    return resolveAfterDelay({
      hero: findMovieOrThrow(HERO_MOVIE_ID),
      rows: [
        ...(promoMovies.length > 0
          ? [{ title: 'โปรโมชั่น', titleEn: 'Promotion', movies: promoMovies }]
          : []),
        ...MOCK_HOME_ROWS.map((row) => ({
          title: row.title,
          titleEn: row.titleEn,
  ...
  getSimilar: (id: string) => {
    ...
    // "แนวเดียวกัน" หมายถึงมี genre ทับกันอย่างน้อย 1 อัน (หนังแต่ละเรื่องมีได้
    // สูงสุด 2 genre อยู่แล้ว)
    const sameGenre = rest.filter((m) => m.genre.some((g) => current.genre.includes(g)));
    const others = rest.filter((m) => !m.genre.some((g) => current.genre.includes(g)));
  ...
  search: (query: string) => {
    ...
    const results = MOCK_MOVIES.filter(
      (m) =>
        ...
        m.subGenre.some((g) => g.toLowerCase().includes(q)) ||
  ...
  getGenres: () =>
    resolveAfterDelay(Array.from(new Set(MOCK_MOVIES.flatMap((m) => m.genre))).sort()),

  // ใหม่: รายชื่อแนวย่อยทั้งหมดที่มีอยู่จริงในคลัง ใช้เรนเดอร์ filter chip คู่กับ genre
  getSubGenres: () =>
    resolveAfterDelay(Array.from(new Set(MOCK_MOVIES.flatMap((m) => m.subGenre))).sort()),
  ...
  // ใหม่: ค้นหาแบบมีตัวกรอง — คำค้น (ไม่บังคับ) + แนวหนัง/แนวย่อย (เลือกได้หลายอัน) + ช่วงราคา
  ...
  searchAdvanced: (params: {
    ...
    const { query = '', genres = [], subGenres = [], minPrice, maxPrice } = params;
    const q = query.trim().toLowerCase();
    ...
    if (genres.length > 0) {
      results = results.filter((m) => genres.some((g) => (m.genre as string[]).includes(g)));
    }
    if (subGenres.length > 0) {
      results = results.filter((m) => subGenres.some((g) => (m.subGenre as string[]).includes(g)));
    }
