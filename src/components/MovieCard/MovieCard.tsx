  ...
  return (
    ...
    >
      <View style={[styles.art, { width, height: width * 1.44 }]}>
        <MoviePoster
          title={localizedTitle(movie, language)}
          genre={localizedGenreList(movie.genre, language)}
          year={movie.year}
        ...
        {owned ? (
          <View style={styles.badgeOwned}>
            <Text style={styles.badgeOwnedText}>{t('ownedBadge')}</Text>
          </View>
        ) : (
          <>
            {hasDiscount && (
              <View style={styles.discountRibbon}>
                <Text style={styles.discountRibbonText}>-{movie.discountPercent}%</Text>
              </View>
            )}
      ...
      <Text style={styles.sub} numberOfLines={1}>
        {localizedGenreList(movie.genre, language)} · {localizedCountry(movie.country, language)}
      </Text>
    </Pressable>
  );
}
